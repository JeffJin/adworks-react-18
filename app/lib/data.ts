...
export async function fetchActivities() {
  try {
    const activities = [
      { month: 'Jan', activities: 2000 },
      { month: 'Feb', activities: 1800 },
      { month: 'Mar', activities: 2200 },
      { month: 'Apr', activities: 2500 },
      { month: 'May', activities: 2300 },
      { month: 'Jun', activities: 3200 },
      { month: 'Jul', activities: 3500 },
      { month: 'Aug', activities: 3700 },
      { month: 'Sep', activities: 2500 },
...
export async function fetchLatestVideos() {
  try {
    ...
    console.error('Database Error:', error);
    throw new Error('Failed to fetch the latest videos.');
  }
}

export async function fetchStatsData() {
  try {
    const numberOfVideos = 569;
    const numberOfCustomers = 33;
    const numberOfImages = 3123;
    const numberOfDocuments = 899;
